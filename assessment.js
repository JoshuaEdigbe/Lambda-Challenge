...
function longestString(arrOfStr) {
  ...
  sortedArr = arrOfStr.sort((a, b) => b.length - a.length);
...
function computeUserAverageAge(arrOfUsers) {
  let sumOfAge = 0,
    average = 0;

  sumOfAge = arrOfUsers.reduce((total, nextUser) => {
    return total + nextUser.age;
  }, 0);

  average = Math.round(sumOfAge / arrOfUsers.length);
